...
#Method to set an inactive customer for logging out
	...
	def change_status():
		...
		with sqlite3.connect("bangazon_cli.db") as bang:
			...
			cursor.execute("UPDATE Customers SET is_active = 'True' WHERE customer_id = '{}'".format(newChoice))
	...
	def get_active_customer(): 
		with sqlite3.connect("bangazon_cli.db") as bang:
			...
			cursor.execute("SELECT customer_id FROM Customers c WHERE c.is_active ='True'")
			data = cursor.fetchall()
			return data[0][0] 
